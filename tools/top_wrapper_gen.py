def gen(f):
    ...
    for i in range(num_bram):
        y0 = rows-(2*i)
        add_inpass(bram_x, y0, "FAB2RAM_A1", f"bram{i}_rd_addr", 0)
        add_inpass(bram_x, y0, "FAB2RAM_A0", f"bram{i}_rd_addr", 4)
        add_inpass(bram_x, y0-1, "FAB2RAM_A1", f"bram{i}_wr_addr", 0)
        add_inpass(bram_x, y0-1, "FAB2RAM_A0", f"bram{i}_wr_addr", 4)
        for i in range(8):
            add_inpass(bram_x, y0-(i//4), f"FAB2RAM_D{3-(i%4)}", f"bram{i}_wr_data", 4*i)
            add_outpass(bram_x, y0-(i//4), f"RAM2FAB_D{3-(i%4)}", f"bram{i}_rd_data", 4*i)
        add_inpass(bram_x, y0, "FAB2RAM_C", f"bram{i}_config", 0)
        add_inpass(bram_x, y0-1, "FAB2RAM_C", f"bram{i}_config", 4)
        print("", file=f)
